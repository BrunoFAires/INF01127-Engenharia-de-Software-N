            ...
            {loading ? <Row justify={"center"}><Spin/></Row> : <>
                ...
                <Form layout="vertical" className="mb-4">
                    <Row gutter={[16, 16]} className="flex-wrap">
                        ...
                        <Col xs={24} sm={12} md={8} lg={4}>
                            <Input placeholder="Preço" size="large"
                                   onChange={e => handleFilterChange('price', e.target.value)} className="w-full"/>
                        </Col>
                        <Col xs={24} sm={12} md={8} lg={4}>
                            <Input placeholder="Artista" size="large"
                                   onChange={e => handleFilterChange('artist', e.target.value)} className="w-full"/>
                        </Col>
                        <Col xs={24} sm={12} md={8} lg={4}>
                            <Input placeholder="Raridade" size="large"
                                   onChange={e => handleFilterChange('rarity', e.target.value)} className="w-full"/>
                        </Col>
                        <Col xs={24} sm={12} md={8} lg={4}>
                            <Input placeholder="Pokémon" size="large"
                                   onChange={e => handleFilterChange('name', e.target.value)} className="w-full"/>
                        </Col>
                        <Col xs={24} sm={12} md={8} lg={4}>
                            <Select
                    ...
                    <Row gutter={[16, 16]} style={{marginTop: '24px'}}>
                        {ads.map(ad => (
                            <Col xs={24} sm={12} md={8} lg={6} xl={4} key={ad.id}>
                                <Card
                                    hoverable
                                    cover={<img alt={ad.card.name} src={ad.card.image}/>}
                                ...
                                >{console.log(ad)}
                                    <Card.Meta
                                        ...
                                        description={(
                                            <div className="mt-2">
                                                <div className="mt-2"><Text strong
                                                                            className="text-gray-600">Descrição: </Text><Text
                                                    className="text-black">{`${ad.description.slice(0, 35)}${ad.description.length > 30 ? '...' : ''}`}</Text>
                                                </div>
                                                <div className="mt-2"><Text strong
                                                                            className="text-gray-600">Preço: </Text><Text
                                                    className="text-black">R$ {ad.price}</Text></div>
                                                <div className="mt-1"><Text strong
                                                                            className="text-gray-600">Artista: </Text><Text
                                                    className="text-black">{ad.card.artist}</Text></div>
                                                <div className="mt-1"><Text strong
                                                                            className="text-gray-600">Raridade: </Text><Text
                                                    className="text-black">{ad.card.rarity}</Text></div>
                                                <div className="mt-1"><Text strong
                ...
                <Row justify={'end'} className='pb-3'>
                    <Button size={'large'} type="primary" onClick={() => navigate('/community')}>Acessar
                        Comunidade</Button>
                </Row>
                <div
                    className="flex items-center h-[50vh] overflow-x-auto overflow-clip space-x-3 shadow-lg mb-10 py-2">
