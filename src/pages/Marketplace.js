import {Button, Card, Carousel, Col, Layout, Modal, Row, Spin, Typography} from 'antd';
                    ...
                    advertisement ? (
                        <Content bordered={false}>
                            <Row className='flex flex-col md:flex-row'>
                                <Col sspan={24} md={12} className='flex flex-col items-center justify-around'>
                                    <span className="text-2xl font-bold">{advertisement.title}</span>
                                    <img src={advertisement.card.image} alt="Card"
                                ...
                                <Col span={24} md={12} className='gutter-row'>
                                    <div className=' space-y-3 justify-center'>
                                        <p className="">{`Descrição: ${advertisement.description}`}</p>
                                        <p className="">{`Vendedor: ${advertisement.seller.name}`}</p>
                                        <div className="mt-1">
                                            <Text>Status: </Text><Text
                                            class={`${advertisement.status === 1 ? 'text-green-500' : 'text-red-500'}`}>{advertisement.status === 1 ? 'Ativo' : 'Inativo'}</Text>
                                        </div>
                                        <p className="">{`Disponível em estoque: ${advertisement.quantity}`}</p>
                            ...
                            {advertisement?.sale ? (
                                ...
                                <Row className='flex flex-col md:flex-row mt-3 justify-around w-full p-4'>
                                    ...
                                    <Col span={24} md={12}
                                         className='gutter-row mt-4 flex justify-center md:mt-0 md:justify-start'>
