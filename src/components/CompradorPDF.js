...
const CompradorPDF = ({ Orders }) => {
  const { toPDF, targetRef } = usePDF({ filename: 'page.pdf' });

  console.log(Orders)
  return (
    <div>
      <button onClick={() => toPDF()}>Gerar Recibo Compra</button>
      <div ref={targetRef} style={{ position: 'absolute', left: '-9999px', padding: '20px', backgroundColor: '#f5f5f5', width: '210mm', height: '296mm' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
          <div style={{ flex: 1, paddingRight: '10px' }}>
            ...
            <Text strong>Nome:</Text> <Text>{Orders.order_user[0].advertisement.seller.name} {Orders.order_user[0].advertisement.seller.surname}</Text><br />
            <Text strong>Avaliação do Vendedor:</Text> <Text>{Orders.order_user[0].advertisement.seller.ratings}</Text><br />
